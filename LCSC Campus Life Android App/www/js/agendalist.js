...
$.extend(ListView.prototype, {
    ...
    renderHtml : function() {
        return '<ul class="fc-agendaList"></ul>';
    },
    setHeight: function(totalHeight, isAuto) {
    /*
		var eventLimit;
            ...
            for (i in displayeventlist) {
               ...
               //if ( evMonth >= visMonth && evYear == visYear) {
				  if (xxy >= xcurrentDate) {	
                    ...
                    startDate       = htmlEscape(formatDate(displayeventlist[i].start, 'h:mm a'));
                    if ( displayeventlist[i].end ) {
                        endDate         = htmlEscape(formatDate(displayeventlist[i].end, 'h:mm a'));
                    }
                    lurl        = htmlEscape(displayeventlist[i].id);
                    classes     = displayeventlist[i].className;
                    description = displayeventlist[i].description;
                    var location = displayeventlist[i].location;
...
$.extend(AgendaListView.prototype, { 
	...
	render  : function(date) {
        this.intervalStart = date.clone().stripTime().startOf('month');
		this.intervalEnd = this.intervalStart.clone().add('months', 4);
		this.start = this.intervalStart.clone();
		this.end = this.intervalEnd.clone();
        this.title = this.calendar.formatDate(this.start, this.opt('titleFormat'));
